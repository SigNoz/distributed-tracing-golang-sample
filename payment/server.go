...
func setupServer() {
	...
	log.Printf("Payment service running on port: %d", httpPort())
...
func httpPort() int {
	p, ok := os.LookupEnv("PAYMENT_PORT")
	if !ok {
		log.Fatal("PAYMENT_PORT not set")
	}

	port, err := strconv.Atoi(p)
	if err != nil {
		log.Fatalf("incorrect payment port: %v", err)
	}
...
func main() {
	...
	_ = userServicePort() // validation of port
	initDB()
	go setupServer()
